...
/// <summary>
/// Service for direct message conversation operations
/// </summary>
public class ConversationService : IConversationService
{
    private readonly IApiClient _apiClient;

    public ConversationService(IApiClient apiClient)
    {
    ...
    public async Task<Result<Guid>> StartConversationAsync(Guid otherUserId)
    {
        var response = await _apiClient.PostAsync<StartConversationResponse>(
            "api/conversations",
            new StartConversationRequest { OtherUserId = otherUserId });
    ...
    public async Task<Result<Guid>> SendMessageAsync(Guid conversationId, string content, string? fileId = null)
    {
        var response = await _apiClient.PostAsync<SendMessageResponse>(
            $"api/conversations/{conversationId}/messages",
            new SendMessageRequest { Content = content, FileId = fileId });
    ...
    public async Task<Result> EditMessageAsync(Guid conversationId, Guid messageId, string newContent)
    {
        return await _apiClient.PutAsync(
            ...
            new EditMessageRequest { NewContent = newContent });
    }
