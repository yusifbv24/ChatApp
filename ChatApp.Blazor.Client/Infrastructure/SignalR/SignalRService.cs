...
public class SignalRService : ISignalRService, IAsyncDisposable
{
    ...
    {
        // Presence events
        _subscriptions.Add(_hubConnection.On<Guid>("UserOnline", userId =>
        {
            OnUserOnline?.Invoke(userId);
        }));

        _subscriptions.Add(_hubConnection.On<Guid>("UserOffline", userId =>
        {
            OnUserOffline?.Invoke(userId);
        }));

        // Direct message events
        _subscriptions.Add(_hubConnection.On<object>("NewDirectMessage", messageObj =>
        {
            try
            {
        ...
        // Channel message events
        _subscriptions.Add(_hubConnection.On<object>("NewChannelMessage", messageObj =>
        {
            try
            {
                var json = JsonSerializer.Serialize(messageObj);
        ...
        _subscriptions.Add(_hubConnection.On<object>("MessageEdited", data =>
        {
            try
            {
                ...
                else if (root.TryGetProperty("conversationId", out var convIdProp) &&
        ...
        _subscriptions.Add(_hubConnection.On<object>("MessageDeleted", data =>
        {
            try
            {
                var json = JsonSerializer.Serialize(data);
        ...
        // Message read events
        _subscriptions.Add(_hubConnection.On<object>("MessageRead", data =>
        {
            try
            {
                var json = JsonSerializer.Serialize(data);
        ...
        // Typing indicators
        _subscriptions.Add(_hubConnection.On<Guid, Guid, bool>("UserTypingInChannel", (channelId, userId, isTyping) =>
        {
            OnUserTypingInChannel?.Invoke(channelId, userId, isTyping);
        }));

        _subscriptions.Add(_hubConnection.On<Guid, Guid, bool>("UserTypingInConversation", (conversationId, userId, isTyping) =>
        {
            OnUserTypingInConversation?.Invoke(conversationId, userId, isTyping);
        }));

        // Reaction events
        _subscriptions.Add(_hubConnection.On<object>("ReactionAdded", data =>
        {
            try
            {
                var json = JsonSerializer.Serialize(data);
        ...
        _subscriptions.Add(_hubConnection.On<object>("ReactionRemoved", data =>
        {
            try
            {
                var json = JsonSerializer.Serialize(data);
    ...
    public async Task JoinChannelAsync(Guid channelId)
    {
        if (_isInitialized)
        {
            await _hubConnection.SendMessageAsync("JoinChannel", channelId);
        }
    }
    ...
    public async Task LeaveChannelAsync(Guid channelId)
    {
        if (_isInitialized)
        {
            await _hubConnection.SendMessageAsync("LeaveChannel", channelId);
        }
    }
    ...
    public async Task JoinConversationAsync(Guid conversationId)
    {
        if (_isInitialized)
        {
            await _hubConnection.SendMessageAsync("JoinConversation", conversationId);
        }
    }
    ...
    public async Task LeaveConversationAsync(Guid conversationId)
    {
        if (_isInitialized)
        {
            await _hubConnection.SendMessageAsync("LeaveConversation", conversationId);
        }
    }
    ...
    public async Task SendTypingInChannelAsync(Guid channelId, bool isTyping)
    {
        if (_isInitialized)
        {
            await _hubConnection.SendMessageAsync("TypingInChannel", channelId, isTyping);
        }
    }
    ...
    public async Task SendTypingInConversationAsync(Guid conversationId, bool isTyping)
    {
        if (_isInitialized)
        {
            await _hubConnection.SendMessageAsync("TypingInConversation", conversationId, isTyping);
        }
    }
