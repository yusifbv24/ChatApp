/**
 * Chat Area — replaces ChatArea.razor + scroll management from app.js
...
(function () {
    ...
    // --- Event delegation for @mention clicks on messagesList ---
    // Instead of per-element listeners in createMessageElement
    messagesList.addEventListener('click', function (e) {
        var mentionTag = e.target.closest('.mention-tag[data-user-id]');
        if (mentionTag) {
            var userId = mentionTag.getAttribute('data-user-id');
            if (userId && ChatApp.profilePanel) {
                ChatApp.profilePanel.open(userId);
            }
        }
    });
    ...
    async function openConversation(convId, type, conv) {
        ...
        _myChannelRole = null;
        _newMsgCount = 0;
        // Clear dedup sets on conversation switch to prevent memory leaks
    ...
    function updateHeader(conv) {
        ...
        if (isNotes) {
            // Notes (self-chat) — show bookmark icon
    ...
    function showAddMemberDialog() {
        ...
        var _addMemberUsers = []; // cached search results for delegation

        // Event delegation on resultsDiv for add buttons (avoids per-item listeners)
        resultsDiv.addEventListener('click', async function (e) {
            var addBtn = e.target.closest('.add-member-add-btn');
            if (!addBtn || addBtn.disabled) return;
            var userId = addBtn.dataset.userId;
            if (!userId) return;
        ...
        searchInput.addEventListener('input', ChatApp.utils.debounce(async function () {
            ...
            users.forEach(function (user) {
                ...
                item.innerHTML = ChatApp.utils.renderAvatar({ id: user.id, avatarUrl: user.avatarUrl, fullName: user.fullName }, 'add-member-avatar') +
                    ...
                    '<button class="add-member-add-btn" data-user-id="' + user.id + '" style="border:none;background:#25D366;color:#fff;padding:4px 12px;border-radius:6px;font-size:13px;cursor:pointer;">Add</button>';
    ...
    function createMessageElement(msg, isGrouped) {
        ...
        if (msg.replyToMessage || msg.repliedMessage) {
            ...
            if (reply.fileId || (reply.files && reply.files.length > 0)) {
                var replyFileName = reply.fileName || (reply.files && reply.files[0] ? reply.files[0].fileName : null) || 'File';
                replyDiv.innerHTML += '<span class="reply-file-info"><span class="material-icons" style="font-size:14px;">attach_file</span> ' +
                    ChatApp.utils.escapeHtml(replyFileName) + '</span>';
            }
        ...
        if (msg.isPinned) {
            var pinnedInline = document.createElement('span');
            pinnedInline.className = 'material-icons pinned-icon-inline';
            pinnedInline.style.fontSize = '12px';
            pinnedInline.textContent = 'push_pin';
            statusRow.appendChild(pinnedInline);
        }
        if (isOwn && (msg.status !== undefined && msg.status !== null)) {
            const statusIcon = document.createElement('span');
            statusIcon.className = 'message-status-icon';
            if (msg.status === 'Failed') {
                statusIcon.innerHTML = '<span class="material-icons message-status-icon failed" style="font-size:14px;color:#e53935;">error_outline</span>';
            } else if (msg.status === 'Pending' || msg.status === 0) {
                statusIcon.innerHTML = '<span class="material-icons message-status-icon pending" style="font-size:14px;color:#999;">schedule</span>';
            } else if (msg.status === 'Sent' || msg.status === 1) {
                statusIcon.innerHTML = '<span class="material-icons" style="font-size:14px;">done</span>';
            } else if (msg.status === 'Delivered' || msg.status === 2) {
                statusIcon.innerHTML = '<span class="material-icons" style="font-size:14px;">done_all</span>';
            } else if (msg.status === 'Read' || msg.status === 3) {
                statusIcon.innerHTML = '<span class="material-icons" style="font-size:14px;color:#4fc3f7;">done_all</span>';
            } else {
                statusIcon.innerHTML = '<span class="material-icons" style="font-size:14px;">done</span>';
            }
        ...
        if (msg.reactions && msg.reactions.length > 0) {
            ...
            msg.reactions.forEach(function (r) {
                ...
                if (r.users && r.users.length > 0) {
                    ...
                    if (_currentConvType === 'channel') {
                        // Rich avatar panel for channel messages
                        tooltip.innerHTML = '';
                        r.users.forEach(function (u) {
                            var userRow = document.createElement('div');
                            userRow.className = 'reaction-tooltip-user';
                            userRow.style.cssText = 'display:flex;align-items:center;gap:6px;padding:3px 0;';
                            userRow.innerHTML = ChatApp.utils.renderAvatar(
                                { id: u.userId || u.id, avatarUrl: u.avatarUrl, fullName: u.fullName || u.userName || '' },
                                'reaction-tooltip-avatar') +
                                '<span style="font-size:12px;">' + ChatApp.utils.escapeHtml(u.fullName || u.userName || '') + '</span>';
                            tooltip.appendChild(userRow);
                        });
                    } else {
        ...
        // @mention click handlers moved to event delegation on messagesList (see below)
    ...
    // --- File attachment ---
    // File icon mapping — delegates to cached utils version
    function getFileIcon(fileName) {
        return ChatApp.utils.getFileIcon(fileName);
    }
    ...
    function renderLinkPreviews(msgEl, content) {
        ...
        urls.forEach(function (url) {
            ...
            ChatApp.api.get('/api/files/link-preview?url=' + encodeURIComponent(url))
    ...
    function showMessageContextMenu(e, msg, chevronMenu) {
        ...
        var isFileOnly = !msg.content && msg.files && msg.files.length > 0;
        if (isOwn && !msg.forwardedFromMessageId && !isFileOnly) actions.push({ action: 'edit', icon: 'edit', text: 'Edit' });
        if (msg.files && msg.files.length > 0 && (!ChatApp.state.hasPermission || ChatApp.state.hasPermission('Files.Download'))) {
    ...
    // --- Add pending upload message to UI ---
    function addPendingUpload(tempId, content, files, onCancel) {
        var el = document.createElement('div');
        el.className = 'message-bubble-wrapper own pending-upload';
        el.dataset.tempId = tempId;
        el.dataset.messageId = 'upload-' + tempId;
        ...
        timeSpan.className = 'message-time';
        timeSpan.textContent = ChatApp.utils.formatMessageTime(new Date().toISOString());
        senderRow.appendChild(timeSpan);
        contentCol.appendChild(senderRow);
        ...
        filesContainer.className = 'message-files pending-upload-files';
        var progressElements = {};

        files.forEach(function (file) {
            var fileEl = document.createElement('div');
            fileEl.className = 'pending-upload-file-item';
            var fileId = file._trackId || ChatApp.utils.generateId();
            fileEl.dataset.fileId = fileId;
            var isImage = /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(file.name);

            if (isImage) {
                var imgWrapper = document.createElement('div');
                imgWrapper.className = 'pending-upload-image-wrapper';
                var img = document.createElement('img');
                img.className = 'pending-upload-image';
                ...
                var overlay = document.createElement('div');
                overlay.className = 'pending-upload-overlay';
                var svgNS = 'http://www.w3.org/2000/svg';
                var svg = document.createElementNS(svgNS, 'svg');
                svg.setAttribute('class', 'pending-upload-circle-progress');
                ...
                fgCircle.setAttribute('cx', '24');
                fgCircle.setAttribute('cy', '24');
                fgCircle.setAttribute('r', '20');
                fgCircle.setAttribute('class', 'progress-fg');
                fgCircle.style.strokeDasharray = '125.66';
                fgCircle.style.strokeDashoffset = '125.66';
                svg.appendChild(fgCircle);
                overlay.appendChild(svg);
                var percentText = document.createElement('span');
                percentText.className = 'pending-upload-percent';
                percentText.textContent = '0%';
                overlay.appendChild(percentText);
                imgWrapper.appendChild(overlay);
                fileEl.appendChild(imgWrapper);

                progressElements[fileId] = { type: 'image', circle: fgCircle, text: percentText, overlay: overlay, wrapper: imgWrapper };
            } else {
                var docEl = document.createElement('div');
                docEl.className = 'pending-upload-doc';
                var fi = getFileIcon(file.name);
                var iconSpan = document.createElement('span');
                iconSpan.className = 'material-icons';
                iconSpan.style.cssText = 'font-size:22px;' + fi.color;
                iconSpan.textContent = fi.icon;
                docEl.appendChild(iconSpan);
                var infoDiv = document.createElement('div');
                infoDiv.className = 'pending-upload-doc-info';
                infoDiv.innerHTML = '<span class="file-doc-name">' + ChatApp.utils.escapeHtml(file.name) + '</span>' +
                    '<span class="file-doc-size">' + ChatApp.utils.formatFileSize(file.size) + '</span>';
                docEl.appendChild(infoDiv);
                var cancelBtn = document.createElement('button');
                cancelBtn.className = 'pending-upload-cancel-btn';
                cancelBtn.title = 'Cancel upload';
                cancelBtn.innerHTML = '<span class="material-icons" style="font-size:16px;">close</span>';
                cancelBtn.addEventListener('click', function () { if (onCancel) onCancel(tempId); });
                docEl.appendChild(cancelBtn);
                fileEl.appendChild(docEl);

                var progressBar = document.createElement('div');
                progressBar.className = 'pending-upload-progress-bar';
                var progressFill = document.createElement('div');
                progressFill.className = 'pending-upload-progress-fill';
                progressFill.style.width = '0%';
                progressBar.appendChild(progressFill);
                fileEl.appendChild(progressBar);

                progressElements[fileId] = { type: 'doc', fill: progressFill, cancelBtn: cancelBtn };
            }
        ...
        statusRow.className = 'message-status-row';
        statusRow.innerHTML = '<span class="material-icons message-status-icon pending" style="font-size:14px;color:#999;">schedule</span>';
        bubble.appendChild(statusRow);
        contentCol.appendChild(bubble);
        ...
        return {
            updateProgress: function (percent, fileId) {
                if (fileId && progressElements[fileId]) {
                    var pe = progressElements[fileId];
                    if (pe.type === 'image') {
                        var offset = 125.66 * (1 - percent / 100);
                        pe.circle.style.strokeDashoffset = offset;
                        pe.text.textContent = Math.round(percent) + '%';
                        pe.wrapper.querySelector('.pending-upload-image').style.filter = 'blur(' + Math.max(0, 4 * (1 - percent / 100)) + 'px)';
                    } else {
                        pe.fill.style.width = percent + '%';
                    }
                } else {
                    Object.keys(progressElements).forEach(function (fid) {
                        var pe = progressElements[fid];
                        if (pe.type === 'image') {
                            var offset = 125.66 * (1 - percent / 100);
                            pe.circle.style.strokeDashoffset = offset;
                            pe.text.textContent = Math.round(percent) + '%';
            ...
            fail: function (error) {
                ...
                var sr = el.querySelector('.message-status-row');
                if (sr) {
                    sr.innerHTML = '<span class="material-icons" style="font-size:14px;color:#e53935;">error_outline</span>' +
                        '<span style="font-size:12px;color:#e53935;margin-left:4px;">' + ChatApp.utils.escapeHtml(error || 'Upload failed') + '</span>';
                }
                var retryBtn = document.createElement('button');
                retryBtn.className = 'pending-upload-retry-btn';
                retryBtn.innerHTML = '<span class="material-icons" style="font-size:14px;">refresh</span> Retry';
                retryBtn.addEventListener('click', function () {
                    el.remove();
                    ChatApp.state.emit('retryUpload', { tempId: tempId, content: content, files: files });
                });
                var bbl = el.querySelector('.message-bubble');
                if (bbl) bbl.appendChild(retryBtn);
                Object.keys(progressElements).forEach(function (fid) {
                    var pe = progressElements[fid];
                    if (pe.type === 'image' && pe.overlay) {
                        pe.overlay.innerHTML = '<span class="material-icons" style="font-size:24px;color:#e53935;">error</span>';
                    } else if (pe.type === 'doc' && pe.fill) {
                        pe.fill.style.width = '100%';
                        pe.fill.style.background = '#e53935';
                    }
                });
            }
        };
    ...
    function updateReactions(messageId, reactions) {
        ...
        if (idx !== -1) {
            ...
            // Targeted update: replace only the affected message element instead of full re-render
            var existingEl = messagesList.querySelector('[data-message-id="' + messageId + '"]');
            if (existingEl) {
                var isGrouped = existingEl.classList.contains('grouped');
                var newEl = createMessageElement(_messages[idx], isGrouped);
    ...
    // Read status — targeted DOM update instead of full re-render
    ChatApp.signalR.on('messageRead', function (data) {
        ...
        if (msg) {
            msg.status = 'Read';
            msg.messageStatus = 2;
            // Update just the status icon in-place
            var el = messagesList.querySelector('[data-message-id="' + data.messageId + '"]');
            if (el) {
                var statusIcon = el.querySelector('.message-status-icon');
                if (statusIcon) {
                    statusIcon.innerHTML = '<span class="material-icons" style="font-size:14px;color:#4fc3f7;">done_all</span>';
                }
            }
    ...
    // --- User online/offline — update chat header status ---
    ChatApp.signalR.on('userOnline', function (data) {
        var userId = data.userId || data;
        if (!_currentConvId || _currentConvType === 'channel') return;
        if (_currentConv && _currentConv.otherUserId === userId) {
            chatHeaderStatus.textContent = 'Online';
            chatHeaderStatus.className = 'chat-header-status online';
        }
    });

    ChatApp.signalR.on('userOffline', function (data) {
        var userId = data.userId || data;
        if (!_currentConvId || _currentConvType === 'channel') return;
        if (_currentConv && _currentConv.otherUserId === userId) {
            var lastSeen = data.lastSeen || new Date().toISOString();
            chatHeaderStatus.textContent = 'Last seen ' + ChatApp.utils.formatRelativeTime(lastSeen);
            chatHeaderStatus.className = 'chat-header-status';
        }
    });

    // --- Channel messages read — update read receipts ---
    ChatApp.signalR.on('channelMessagesRead', function (data) {
        if (data.channelId !== _currentConvId) return;
        // data.messageReadCounts is an array of { messageId, readByCount } or a dict
        var readCounts = data.messageReadCounts;
        if (!readCounts) return;
        var changed = false;
        // Handle both array and object formats
        if (Array.isArray(readCounts)) {
            readCounts.forEach(function (rc) {
                var msg = _messages.find(function (m) { return m.id === rc.messageId; });
                if (msg) {
                    msg.readByCount = rc.readByCount || (msg.readByCount || 0) + 1;
                    changed = true;
                }
            });
        } else if (typeof readCounts === 'object') {
            Object.keys(readCounts).forEach(function (msgId) {
                var msg = _messages.find(function (m) { return m.id === msgId; });
                if (msg) {
                    msg.readByCount = readCounts[msgId];
                    changed = true;
                }
            });
        }
        // Targeted DOM updates for read-by counts instead of full re-render
        if (changed) {
            var updatedMsgIds = [];
            if (Array.isArray(readCounts)) {
                readCounts.forEach(function (rc) { updatedMsgIds.push(rc.messageId); });
            } else if (typeof readCounts === 'object') {
                updatedMsgIds = Object.keys(readCounts);
            }
            updatedMsgIds.forEach(function (msgId) {
                var msg = _messages.find(function (m) { return m.id === msgId; });
                if (!msg) return;
                var el = messagesList.querySelector('[data-message-id="' + msgId + '"]');
                if (el) {
                    var readByDiv = el.querySelector('.message-read-by');
                    if (readByDiv) {
                        readByDiv.textContent = 'Read by ' + msg.readByCount;
                    } else {
                        var bubble = el.querySelector('.message-bubble');
                        if (bubble) {
                            readByDiv = document.createElement('div');
                            readByDiv.className = 'message-read-by';
                            readByDiv.textContent = 'Read by ' + msg.readByCount;
                            bubble.appendChild(readByDiv);
                        }
    ...
    // --- Member left channel ---
    ChatApp.signalR.on('memberLeftChannel', function (data) {
        if (data.channelId !== _currentConvId) return;
        var currentUserId = ChatApp.state.currentUser ? ChatApp.state.currentUser.id : null;
        if (data.userId === currentUserId) {
            _isMember = false;
            _myChannelRole = null;
            if (joinChannelBanner) joinChannelBanner.style.display = '';
        }
        // Update member count in header
        if (_currentConv && _currentConv.memberCount) {
            _currentConv.memberCount = Math.max(0, _currentConv.memberCount - 1);
            chatHeaderStatus.textContent = _currentConv.memberCount + ' members';
        }
    });
